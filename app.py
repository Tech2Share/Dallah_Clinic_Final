...
def merge_and_process_data(file_1, file_2, output_csv_file, clinic_id, date):
    ...
    try:
        ...
        final_merged_df['Total_number_of_slots'] = 832  # Set total slots as 780 for all doctors
          # Set Complaint column to 'adjusted' for all rows
...
def submit():
 ...
 # Install this library using `pip install python-magic` for file type detection

# Function to check and assign the extension based on the file type
def assign_extension(file):
    if not file.filename:
        return None  # No filename (file is empty)

    # Check if file has an extension
    file_name, file_extension = os.path.splitext(file.filename)

    if file_extension:  # If file already has an extension, return it as is
        return file.filename

    # If no extension, detect file type and assign appropriate extension
    mime = magic.Magic(mime=True)  # Use python-magic to get MIME type
    mime_type = mime.from_buffer(file.read(1024))  # Read the first 1024 bytes of the file

    # Assign extensions based on MIME type
    if 'html' in mime_type:
        return file_name + '.html'
    elif 'csv' in mime_type:
        return file_name + '.csv'
    else:
        return file.filename  # Default, keep original filename

# Modify your Flask route to handle files without extensions
...
def index():
    if request.method == "POST":
        # Retrieve the form data (make sure these are all present in the HTML form)
        clinic = request.form.get('clinic')  # Get 'clinic' from the form (use .get() to avoid KeyError)
        date_str = request.form.get('date')  # Get 'date' from the form (use .get() to avoid KeyError)
        
        if not clinic or not date_str:
            return "Error: Clinic or Date is missing from the form!"

        try:
            date = datetime.datetime.strptime(date_str, '%Y-%m-%d').date()  # Convert date to datetime object
        except ValueError:
        ...
        if not ab_file or not ac_file:
            return "Error: Missing one or more file uploads."

        # Define file paths
        ab_file_path = os.path.join("uploads", assign_extension(ab_file))
        ac_file_path = os.path.join("uploads", assign_extension(ac_file))
        ...
        # Add the complaints, complaint type, and remarks columns to the DataFrame
        complaints = request.form.get('complaints')
        complaint_type = request.form.get('complaint_type')
        complaint_remarks = request.form.get('complaint_remarks')
        referral_patients = request.form.get('referral_patients')

        df['complaints'] = complaints
        df['complaint_type'] = complaint_type
        df['complaint_remarks'] = complaint_remarks
        df['referral_patients'] = referral_patients

        # Save the updated master CSV file
        df.to_csv(master_csv_path, index=False)

        # Upload the final file to Google Sheets
...
@app.route("/success")
def success():
    # Get clinic and date from the URL query parameters
    clinic = request.args.get('clinic')
    date = request.args.get('date')

    # Render the success template and pass clinic and date as context
    return render_template('success.html', clinic=clinic, date=date)
